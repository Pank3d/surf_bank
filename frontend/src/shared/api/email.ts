...
export interface SendTemplateEmailParams {
	to: string;
	template: 'welcome' | 'forgot-password' | 'contact' | 'company-info';
	data?: {
		name?: string;
		email?: string;
		phone?: string;
...
export interface SendCompanyInfoEmailParams {
	to: string;
...
export const sendEmail = async (
	...
	try {
		const { data } = await emailServiceApi.post<EmailResponse>(
			'/send-email',
			params,
		);
		return data;
...
export const useSendContactEmail = () => {
	return useMutation({
		mutationFn: async (formData: {
			...
			const contactEmail =
				import.meta.env.VITE_CONTACT_EMAIL || 'contact@surfbank.com';
...
export const useSendCompanyInfoEmail = () => {
	return useMutation({
		mutationFn: async (formData: {
			expectedVolume: string;
			region: string;
