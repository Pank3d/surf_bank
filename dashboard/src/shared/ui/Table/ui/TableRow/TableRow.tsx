...
interface CryptoProps extends BaseProps {
	isFiat: false;
...
interface FiatProps extends BaseProps {
	isFiat: true;
	received_amount?: never;
	fee?: never;
	sender?: never;
	receiver?: never;
}

type Props = CryptoProps | FiatProps;

export const TableRow = (props: Props) => {
	const [isTableOpen, setIsTableOpen] = useState(false);
	const { id, date, transfer_amount, type, status, isFiat } = props;
	...
	return (
		<>
			<tr onClick={toggleTable} className={style.row}>
				<td className={clsx(style.cell, isFiat && style.widerCell)}>{id}</td>

				<td className={clsx(style.cell, isFiat && style.widerCell)}>{date}</td>

				<td className={clsx(style.cell, isFiat && style.widerCell)}>
					{transfer_amount}
				</td>

				{!isFiat && <td className={style.cell}>{props.received_amount}</td>}

				<td className={clsx(style.cell, isFiat && style.widerCell)}>{type}</td>

				{!isFiat && <td className={style.cell}>{props.fee}</td>}

				<td
					className={clsx(
						style.cell,
						isFiat && style.widerCell,
						style.statusCell,
					)}
				>
			...
			{!isFiat && isTableOpen && props.sender && props.receiver && (
				<tr className={style.row__details}>
					...
					<td className={style.cell} colSpan={4}>
						...
						<span className={style.name}>{props.receiver}</span>
					</td>
				</tr>
			)}
		</>
