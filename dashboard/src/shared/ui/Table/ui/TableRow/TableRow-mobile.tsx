...
export const TableRowMobile = (props: Props) => {
	const [isTableOpen, setIsTableOpen] = useState(false);
	const { id, date, status, transfer_amount, type, isFiat } = props;
	...
	return (
		<>
			<tr onClick={toggleTable} className={style.row}>
				...
				<td className={clsx(style.cell, style.statusCell)}>
					<div className={style.cell__inner}>
						<span>{status}</span>

						{!isFiat && (
							<button
								className={clsx(style.button, isTableOpen && style.active)}
							>
								<svg
									width='10'
									height='6'
				...
				<>
					<tr className={style.row__details}>
						...
						<td className={style.cell}>
							...
							<span>{props.received_amount}</span>
						</td>
					</tr>
					<tr className={style.row__details}>
						<td className={style.cell}>
